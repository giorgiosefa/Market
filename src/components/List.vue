<template>
    <div class="my-3 p-3 bg-white rounded shadow-sm">
        <h6 class="border-bottom border-gray pb-2 mb-0">{{Market}}</h6>
        <div v-for="company in Companies" :key="company.id" class="media text-muted pt-3">
          ...
          <div class="media-body pb-4 mb-0 small lh-125 border-bottom border-gray">
            <div class="d-flex justify-content-between align-items-center w-100">
              <strong class="text-gray-dark mt-1">{{company.name}}</strong>
              <a href="#">{{company.price}}</a>
            </div>
          </div>
        </div>
